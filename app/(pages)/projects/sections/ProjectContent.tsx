...
export const ProjectContent = ({ id }: { id: string }) => {
      ...
      fetch(`/api/projects/${id}`)
        ...
        .then((data) => {
          if (data.success && data.project) {
            setMarkdownContent(data.project)
          }
        })
        .catch((err) => {
  ...
  const enhancedContent = markdownContent
    ? {
        ...content,
        description: markdownContent.content, // Use markdown content as description
        tldr: markdownContent.tldr || content?.tldr, // Prefer markdown tldr if available
      }
    : content
    ...
    <section className="bg-project-page-gradient">
      <div className="flex flex-col">
        <Divider.Section className="flex flex-col items-center">
          <AppContent className="flex flex-col gap-12 py-16">
            <div
              className={cn(
                ...
                (enhancedContent?.description as string)?.length > 0
                    ...
                    <div className="flex flex-col gap-6 text-left">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {enhancedContent?.tldr && (
                          ...
                          >
                            {enhancedContent?.tldr as string}
                          </Markdown>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col w-full gap-6 mt-6 md:gap-10 md:mt-10">
                    ...
                    <div className="flex flex-col w-full text-base font-normal leading-relaxed">
                      {(enhancedContent?.description as string)?.length > 0 && (
                        ...
                        >
                          {enhancedContent?.description as string}
                        </Markdown>
                      )}
          ...
          <DiscoverMoreProjects project={project as ProjectInterface} />
        </Divider.Section>
      </div>
    </section>
  )
